...
export function buildPersonaSystem(persona: Persona, product: ActiveProduct): string {
  ...
  const goodTypeHint = product.goodType === 'search'
    ? '노트북·모니터·휴대폰·냉장고·이어폰 같은 카테고리는 사치품이 아니라 한국 가정의 보편적 구매물이다. 스펙·가격·리뷰 비교는 쉽지만, 그렇다고 안 사는 것이 기본값은 아니다. 예산이 감당되고 이 카테고리에 어느 정도(40 이상) 관심이 있으면, 살 시점이 되었을 때 보통 산다. 가격이 비싸 보이면 더 싼 대체 모델을 적극 검토하되, 관심도가 매우 높고(70 이상) 정말 필요하면 다소 비싸도 결국 구매한다.'
...
- 가격이 부담될 때: 탐색재는 더 싼 대체 모델을 적극 검토하지만, 관심도 70 이상이고 정말 필요한 경우에는 그래도 구매한다(이번에 안 사면 결국 곧 사야 하니까). 경험재는 브랜드·경험 가치로 어느 정도 감수한다.
